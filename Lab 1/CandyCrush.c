...
Position *checkCandies(Board *tablero,code *statusCode){
...
int checkBoard(Board *tablero,code *statusCode){
	...
	if (verificarFila(tablero) == 0){
		return 0;
	}
	else if (verificarColumna(tablero) == 0){
		return 0;
	}
...
void saveBoard(Board *tablero, int *id,code *statusCode){
	...
	fprintf(archivoSalida,"%d ",tablero->alto);
	//Se imprime dato de alto
	fprintf(archivoSalida, "%d",tablero->ancho);
	fprintf(archivoSalida,"%s","\n");
	//Se imprime la fecha
	...
	*id=atoi(numero);
	...
	for(i=0;i<m;i++){
		...
		for (j=0;j<n;j++){
			...
			fprintf(archivoSalida,"%c",tablero->Matriz[i][j].letrasCandy);
			fprintf(archivoSalida,"%d",tablero->Matriz[i][j].dureza);
			fprintf(archivoSalida,"%c",tablero->Matriz[i][j].poder);
...
void print(Board *tablero,code *statusCode){
	...
	for(i=0;i<M;i++){
		...
		for (j=0;j<N;j++){
			printf("%s","[");
			printf("%c",tablero->Matriz[i][j].letrasCandy);printf("%s",",");
			printf("%d",tablero->Matriz[i][j].dureza);printf("%s",",");
			printf("%c",tablero->Matriz[i][j].poder);
			printf("%s","]");printf("%s",",");
		}
		printf("%s","]");
		printf("\n");
...
int play(Board *tablero,Position pOri,Position pDes, int *turnos,code *statusCode){
	...
	letraOri=tablero->Matriz[pOri.posY][pOri.posX].letrasCandy;
	durezaOri=tablero->Matriz[pOri.posY][pOri.posX].dureza;
	poderOri=tablero->Matriz[pOri.posY][pOri.posX].poder;
	//A la posOri se le asigna lo de posDes 
	tablero->Matriz[pOri.posY][pOri.posX].letrasCandy = tablero->Matriz[pDes.posY][pDes.posX].letrasCandy;
	tablero->Matriz[pOri.posY][pOri.posX].dureza = tablero->Matriz[pDes.posY][pDes.posX].dureza;
	tablero->Matriz[pOri.posY][pOri.posX].poder = tablero->Matriz[pDes.posY][pDes.posX].poder;
	//A pDes se le asigna los valores de pOri
	tablero->Matriz[pDes.posY][pDes.posX].letrasCandy = letraOri; 
	tablero->Matriz[pDes.posY][pDes.posX].dureza = durezaOri;
	tablero->Matriz[pDes.posY][pDes.posX].poder = poderOri;

	//*******************************************************************************************************************************************
	//Verificar si existen dulces para que se puedan eliminar
	Position *pos;
	pos=checkCandies(tablero,statusCode);
	int p=0;
		if (pos != NULL){
			...
			if(p >= 3){
				ptj=ptj+romperDurezas(tablero,pos,ptj);
			}
	
	//************************************************************************************************************************************************
	
	actualizarTablero(tablero,statusCode);
